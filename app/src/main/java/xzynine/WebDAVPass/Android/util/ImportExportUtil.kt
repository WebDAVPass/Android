...
class ImportExportUtil(
    ...
    private fun createBasicTokenFromUri(uriString: String, ordinal: Long): OtpToken {
        ...
        val tokenType = if (uri.scheme == "otpauth" && uri.host == "totp") {
            xzynine.WebDAVPass.Android.data.OtpTokenType.TOTP
        } else {
            xzynine.WebDAVPass.Android.data.OtpTokenType.HOTP
        }

        // 生成唯一标识符
        val uniqueId = UniqueIdGenerator.generate(secret, algorithm, digits, period)
        
        return xzynine.WebDAVPass.Android.data.OtpToken(
            id = 0,
            ordinal = 0,
            issuer = issuer,
            label = label,
            ...
            counter = 0,
            period = period,
            encryptionType = xzynine.WebDAVPass.Android.data.EncryptionType.NONE,
            uniqueId = uniqueId
        )
    }
