...
data class OtpElement(
    ...
    fun shouldRefreshToken(): Boolean = secondsRemaining == otpModel.period
    ...
    override fun hashCode(): Int = otpModel.hashCode()
    ...
    companion object {
        ...
        fun isValidCounter(counter: Long): Boolean = counter in MIN_HOTP_COUNTER..MAX_HOTP_COUNTER

        fun isValidPeriod(period: Int): Boolean = period in MIN_TOTP_PERIOD..MAX_TOTP_PERIOD

        fun isValidDigits(digits: Int): Boolean = digits in MIN_OTP_DIGITS..MAX_OTP_DIGITS
...
enum class OtpTokenType {
    ...
    companion object {
        fun getFromString(tokenType: String): OtpTokenType =
            when (tokenType.lowercase(Locale.ENGLISH)) {
                "s", "steam" -> STEAM
                "hotp" -> RFC4226
                else -> RFC6238
            }
        ...
        fun getTotpTokenTypeValues(getProprietaryElements: Boolean = true): Array<OtpTokenType> =
            if (getProprietaryElements) {
                arrayOf(RFC6238, STEAM)
            } else {
                arrayOf(RFC6238)
            }

        fun getHotpTokenTypeValues(): Array<OtpTokenType> = arrayOf(RFC4226)
    }
}
